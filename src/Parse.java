...
public class Parse {
			...
			if(minus){
				...
				switch(temp.getTipo()){
					...
					case "tipo_inteiro":
						...
						break;
					case "tipo_string":
						endereco = cache.alocarString(s.getLexema().length() - 1);
	...
	void C() throws Exception{
		...
		if(s.getToken() == tabela.ID){
			...
			int bufferEnd = cache.alocarTempString();
			cache.contTemp += 3; //Deve ser alocado 259 bytes
	...
	String F() throws Exception{
		...
		if(s.getToken() == tabela.OPPAR){
			...
			if(s.getTipo().equals("tipo_string")){
				...
				F_end = cache.contador;
				cache.alocarString(s.getLexema().length() - 1);
			}else{
				String lexTemp = s.getLexema();
				if(s.getLexema().toLowerCase().equals("true"))
					lexTemp = "0FFh";
